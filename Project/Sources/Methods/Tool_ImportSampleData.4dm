...
If (Test path name:C476($booksFile)=Est un document:K24:1) & (Test path name:C476($studentsFile)=Est un document:K24:1)
	$import:=True:C214
Else 
	$import:=False:C215
End if 
...
If ($import)
	...
	If (ok=1)
		Repeat 
			  // from file
			RECEIVE PACKET:C104($doc;$author;Char:C90(Tabulation:K15:37))
			RECEIVE PACKET:C104($doc;$title;Char:C90(Tabulation:K15:37))
			RECEIVE PACKET:C104($doc;$description;Char:C90(Retour chariot:K15:38))
			If (ok=1)
	...
	If (ok=1)
		Repeat 
			  // from file
			RECEIVE PACKET:C104($doc;$lastName;Char:C90(Tabulation:K15:37))
			RECEIVE PACKET:C104($doc;$firstName;Char:C90(Tabulation:K15:37))
			RECEIVE PACKET:C104($doc;$address;Char:C90(Tabulation:K15:37))
			RECEIVE PACKET:C104($doc;$zipCode;Char:C90(Tabulation:K15:37))
			RECEIVE PACKET:C104($doc;$city;Char:C90(Tabulation:K15:37))
			RECEIVE PACKET:C104($doc;$telephone;Char:C90(Tabulation:K15:37))
			RECEIVE PACKET:C104($doc;$email;Char:C90(Tabulation:K15:37))
			RECEIVE PACKET:C104($doc;$grade;Char:C90(Tabulation:K15:37))
			RECEIVE PACKET:C104($doc;$primaryTeacher;Char:C90(Retour chariot:K15:38))
			If (ok=1)
