Case of 
	: (Form event code:C388=Sur chargement:K2:1)
		...
		If (Records in selection:C76([Loans:2])=0)
			...
			OBJECT SET RGB COLORS:C628(*;"status";0xB000;Coul fond transparent:K23:10)  //  green
			<>bookStatus:=Char:C90(Retour chariot:K15:38)+"Book Available"  // <Translation>
			
		Else 
			
			OBJECT SET VISIBLE:C603(*;"Pict_Available";False:C215)
			OBJECT SET VISIBLE:C603(*;"Pict_Unavailable";True:C214)
			OBJECT SET RGB COLORS:C628(*;"status";0x00D00000;Coul fond transparent:K23:10)  //  red
			
			  //Search for the current borrower of the book
			RELATE ONE SELECTION:C349([Loans:2];[Students:3])
			
			<>bookStatus:="Borrowed on "+String:C10([Loans:2]borrowDate:3)+Char:C90(Retour chariot:K15:38)
			<>bookStatus:=<>bookStatus+"by "+[Students:3]firstName:3+" "+[Students:3]lastName:2+"."+Char:C90(Retour chariot:K15:38)
			<>bookStatus:=<>bookStatus+"Return expected on "+String:C10([Loans:2]returnDate:4;Système date court:K1:1)+"."
			
		End if 
		If (Record number:C243([Books:1])=Est un nouvel enregistrement:K29:1)
